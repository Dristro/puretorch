import weakref
import numpy as np

from .device import Device
from .dtype import DTYPES, DType

from typing import Union, Callable

from .context import Context
from .function import Function
from .grad_mode import is_grad_enabled
from .ops import (
    Add,
    Mul,
    Sub,
    Div,
    Neg,
    Pow,
    Exp,
    Log,
    Mean,
    Index,
    MatMul,
    Reshape,
    Transpose,
    VariableSum,
)

Data = Union[int, float, list, tuple, np.ndarray, "Tensor"]


class Tensor:
    def __init__(
        self,
        data: Data,
        requires_grad: bool = False,
        grad_fn: Function | None = None,
        is_leaf: bool = True,
        device: str | Device = "cpu",
    ):
        """
        Creates a Tensor instance with data.

        Initialized public attrs:
            grad: np.ndarray | None
            grad_fn: Function | None
            requires_grad: bool
            is_leaf: bool
            device: device the tensor lives on (cpu only for now)
        Initialized private attrs:
            _data: np.ndarray
            _version: int
            _shape: tuple
            _ndim: int
            _dtype: np.dtype
            _backward_hooks: list
            _self_weakref: weakref.ref

        Args:
            data: data for Tensor
            requires_grad (bool, default=False): enable requires_grad
            grad_fn (Function | None, default=None): function used to arrive
            is_leaf (bool, default=True): is variable leaf
        """

        self._data: np.ndarray = (
            data if isinstance(data, np.ndarray) else np.array(data)
        )
        self.grad: np.ndarray | None = None
        self.grad_fn = grad_fn  # function that produced this variable
        self.requires_grad = requires_grad and is_grad_enabled()
        self.is_leaf = is_leaf
        self._version = 0

        self._shape = self._data.shape
        self._ndim = self._data.ndim
        self._dtype = self._data.dtype  # FIX: change from np.dtype to DType for _to()
        self._backward_hooks = []
        self._self_weakref = weakref.ref(self)  # saved_tensors avoid cycles

        self._device = device

    def _to(self, device: str | Device, dtype: DType):
        # Device
        Warning("[WARNING] tensor.device only supports `cpu`.")
        if isinstance(device, str):
            device = Device(device)
        self._device = device

        # dtype
        self._dtype = dtype
        self._data = self._data.astype(dtype.value)

    @property
    def device(self):
        return self._device

    def item(self):
        return self.data

    def __repr__(self):
        formatted_data = np.array2string(
            self.data,
            precision=4,
            suppress_small=True,
            separator=", ",
            prefix=" " * 7,
        )
        statement = (
            f"tensor({formatted_data}, requires_grad={self.requires_grad})"
            if self.requires_grad
            else f"tensor({formatted_data})"
        )
        return statement

    def zero_grad(self):
        self.grad = None

    def detach(self) -> "Tensor":
        return Tensor(
            self._data.copy(), requires_grad=False, grad_fn=None, is_leaf=True
        )

    def requires_grad_(self, val=True) -> None:
        """In-place requires-grad change"""
        self.requires_grad = val

    @property
    def data(self) -> np.ndarray:
        v = self._data.view()
        v.setflags(write=False)
        return v

    @data.setter
    def data(self, new: Data):
        self._check_inplace_ok()
        new_arr = new if isinstance(new, np.ndarray) else np.array(new)
        self._data = new_arr
        self._bump_version()

    @property
    def shape(self) -> tuple:
        return self._shape

    @property
    def ndim(self) -> int:
        return self._ndim

    @property
    def dtype(self) -> np.dtype:
        return self._dtype

    def numpy(self) -> np.ndarray:
        """convert into numpy.ndarray"""
        return self._data

    def tolist(self) -> list:
        """convert into python-list"""
        return list(self._data)

    def backward(self, gradient: np.ndarray | None = None):
        r"""
        Compute gradients of the graph w.r.t current tensor.
        If `gradient = None`, then its assumed to be one.

        For custom gradients, ensure that: `gradient.shape == tensor.shape`

        Args:
            gradient (np.ndarray | None, default=None): gradient of tensor
        """
        if not self.requires_grad:
            raise RuntimeError(
                "Called .backward() on a tensor that doesn't require grad."
            )

        # assume grad is one if not provided
        if gradient is None:
            gradient = np.ones_like(self._data)

        # build topo sort
        topo = []
        visited = set()

        def build(_tensor: "Tensor"):
            if id(_tensor) in visited:
                return
            visited.add(id(_tensor))
            if _tensor.grad_fn is not None:
                for child in _tensor.grad_fn._parents:
                    build(child)
            topo.append(_tensor)

        build(self)

        # init the grads
        grads = {id(self): gradient.copy()}

        # use topo to propagate
        # reduce number of indent-blocks
        gen = (_t for _t in reversed(topo) if _t.requires_grad)
        for _tensor in gen:
            grad = grads.get(id(_tensor))
            if grad is None:
                continue

            # processing a leaf
            if _tensor.is_leaf:
                if _tensor.grad is None:
                    _tensor.grad = grad  # init grad
                else:  # (or)
                    _tensor.grad += grad  # accumilate grad
                for hook in _tensor._backward_hooks:
                    hook(_tensor)  # call all hooks (if given)

            # processing intermediate tensors
            else:
                # version safety check (detects illegal in-place between forward and backward)
                ctx = _tensor.grad_fn._ctx
                snap = getattr(ctx, "_version_snapshot", None)
                if snap is not None:
                    for parent, seen in zip(_tensor.grad_fn._parents, snap):
                        if parent._version != seen:
                            raise RuntimeError(
                                f"One of the Tensors needed for backward was modified in-place: " \
                                f"saved version {seen}, current version {parent._version} for: {repr(parent)}"
                            )

                grad_out = _tensor.grad_fn.backward(
                    _tensor.grad_fn._ctx, grad
                )  # op-wise backward

                # logic for any function (even custom ones, defined by user)
                if not isinstance(grad_out, tuple):
                    grad_out = (grad_out,)

                # number of params need-not be 2, thus loop
                for parent, g_out in zip(_tensor.grad_fn._parents, grad_out):
                    if g_out is None:
                        continue

                    # inverse-brodcasing, reduce gradient to parent shape (if needed)
                    g_out = _unbroadcast(g_out, parent.shape)

                    if id(parent) not in grads:
                        # put new tensor with grad in dict
                        grads[id(parent)] = g_out

                    else:
                        # accumulate grad, if tensor alr in dict
                        grads[id(parent)] += g_out

    def _bump_version(self):
        self._version += 1

    def _check_inplace_ok(self):
        if self.requires_grad and not self.is_leaf:
            raise RuntimeError(
                "In-place modification on a non-leaf Tensor that requires grad."
            )

    def __len__(self) -> int:
        return len(self.data)

    # hooks
    def register_hook(self, fn: Callable[["Tensor"], None]):
        self._backward_hooks.append(fn)

    # math operators
    def __add__(self, other) -> "Tensor":
        return add(self, other)

    def __radd__(self, other) -> "Tensor":
        return add(other, self)

    def __mul__(self, other) -> "Tensor":
        return mul(self, other)

    def __rmul__(self, other) -> "Tensor":
        return mul(other, self)

    def __sub__(self, other) -> "Tensor":
        return sub(self, other)

    def __rsub__(self, other) -> "Tensor":
        return sub(other, self)

    def __truediv__(self, other) -> "Tensor":
        return div(self, other)

    def __rtruediv__(self, other) -> "Tensor":
        return div(other, self)

    def __neg__(self) -> "Tensor":
        return neg(self)

    def __matmul__(self, other) -> "Tensor":
        return matmul(self, other)

    def __pow__(self, exp: int | float) -> "Tensor":
        return pow(self, exp)

    def __getitem__(self, idx: "int | list | tuple | Tensor") -> "Tensor":
        return index(self, idx)

    # variable comparisons
    def _coerce_other(self, other):
        """Helper to extract data from Tensor or plain number/array."""
        if isinstance(other, Tensor):  # works for subclasses too
            return other.data
        elif isinstance(other, (int, float, np.ndarray, list)):
            return other
        else:
            return NotImplementedError(
                f"other must be either: (int, float, np.ndarray, list), got: {type(other)}"
            )

    def __eq__(self, other) -> bool:
        other = self._coerce_other(other)
        return all(np.equal(self.data, other))

    def __lt__(self, other) -> bool:
        other = self._coerce_other(other)
        return all(np.less(self.data, other))

    def __gt__(self, other) -> bool:
        other = self._coerce_other(other)
        return all(np.greater(self.data, other))

    def __le__(self, other) -> bool:
        other = self._coerce_other(other)
        return all(np.less_equal(self.data, other))

    def __ge__(self, other) -> bool:
        other = self._coerce_other(other)
        return all(np.greater_equal(self.data, other))

    # inplace operators

    def __iadd__(self, other) -> "Tensor":
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            np.add(self._data, other._data, out=self._data, casting="unsafe")
        else:
            np.add(self._data, other, out=self._data, casting="unsafe")
        self._bump_version()
        return type(self)(self)

    def __isub__(self, other) -> "Tensor":
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            np.subtract(self._data, other._data, out=self._data, casting="unsafe")
        else:
            np.subtract(self._data, other, out=self._data, casting="unsafe")
        self._bump_version()
        return type(self)(self)

    def __imul__(self, other) -> "Tensor":
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            np.multiply(self._data, other._data, out=self._data, casting="unsafe")
        else:
            np.multiply(self._data, other, out=self._data, casting="unsafe")
        self._bump_version()
        return type(self)(self)

    def __itruediv__(self, other) -> "Tensor":
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            np.true_divide(self._data, other._data, out=self._data, casting="unsafe")
        else:
            np.true_divide(self._data, other, out=self._data, casting="unsafe")
        self._bump_version()
        return type(self)(self)

    def __ipow__(self, other) -> "Tensor":
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            np.power(self._data, other._data, out=self._data, casting="unsafe")
        else:
            np.power(self._data, other, out=self._data, casting="unsafe")
        self._bump_version()
        return type(self)(self)

    # data logic

    def __setitem__(self, key, value):
        self._check_inplace_ok()
        if isinstance(value, Tensor):
            self._data[key] = value._data
        else:
            self._data[key] = value
        self._bump_version()

    def squeeze(
        self, dim: int | None = None, in_place: bool = True
    ) -> "Tensor | None":
        """
        removes dims with no entries
        Args:
            dim: dimention to squeeze
            in_place: squeeze in-place if True
        """
        return type(self)(
            self.data.squeeze(axis=dim)
            if not in_place
            else self._data.squeeze(axis=dim)
        )

    # tensor functions

    def reshape(self, shape: tuple) -> "Tensor":
        return reshape(self, shape=shape)

    @property
    def T(self) -> "Tensor":
        return transpose(self)

    def sum(
        self,
        dim: int | tuple | None = None,
        keepdims: bool = False,
    ) -> "Tensor":
        return tensor_sum(a=self, dim=dim, keepdims=keepdims)

    def mean(
        self, dim: int | tuple | None = None, keepdims: bool = False
    ) -> "Tensor":
        return mean(a=self, dim=dim, keepdims=keepdims)

    def exp(self) -> "Tensor":
        return exp(a=self)

    def log(self) -> "Tensor":
        return log(a=self)

    def add_(self, other):
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            self._data += other._data
        else:
            self._data += other
        self._bump_version()
        return self

    def mul_(self, other):
        self._check_inplace_ok()
        if isinstance(other, Tensor):
            self._data *= other._data
        else:
            self._data *= other
        self._bump_version()
        return self

    def zero_(self):
        self._check_inplace_ok()
        self._data[...] = 0
        self._bump_version()
        return self


# Helpers for Tensor


def _unbroadcast(grad: np.ndarray, shape: tuple[int, ...]) -> np.ndarray:
    """Reduce grad to the target shape (inverse of broadcasting)."""
    if grad.shape == shape:
        return grad
    # sum over leading dims
    while len(grad.shape) > len(shape):
        grad = grad.sum(axis=0)
    # sum dims of size 1
    for i, (gdim, sdim) in enumerate(zip(grad.shape, shape)):
        if sdim == 1 and gdim != 1:
            grad = grad.sum(axis=i, keepdims=True)
    return grad.reshape(shape)


def enforce_tensor(x) -> Tensor:
    """
    Converts 'x' into a Tensor instance if not already.
    Used for tensor-ops.
    """
    if isinstance(x, Tensor):
        return x
    return Tensor(np.array(x, dtype=float), requires_grad=False, is_leaf=True)


def _wrap_forward(fn_cls: type, *parents, result_cls=None, **kwargs) -> Tensor:
    ctx = Context()
    parents_data = [parent._data for parent in parents]

    # forward
    out_data = fn_cls.forward(ctx, *parents_data, **kwargs)

    # pick output class
    if result_cls is None:
        classes = {type(p) for p in parents}
        if len(classes) == 1:
            result_cls = classes.pop()
        else:
            # pick 'tensor' if any parent is tensor (higher priority to tensor-instances)
            result_cls = next(
                (type(p) for p in parents if p.__class__.__name__ == "Tensor"), Tensor
            )

    # Create output as tensor
    if result_cls.__name__ == "Tensor":
        # FIXED: device issue if parent is mixed (variable/tensor)
        devices = [p.device for p in parents if hasattr(p, "device")]
        out = result_cls(
            data=out_data,
            requires_grad=any(p.requires_grad for p in parents),
            grad_fn=None,
            is_leaf=False,
            device=devices[-1],  # last device from parents
            # FIX-NEEDED: give priority to a device (or) check if all parents are on same device
        )

    # Create output as variable
    else:
        out = Tensor(
            data=out_data,
            requires_grad=any(p.requires_grad for p in parents),
            grad_fn=None,
            is_leaf=False,
        )

    # Bind grad_fn instance, parents, and ctx
    fn = fn_cls()
    fn._ctx = ctx
    fn._parents = parents
    ctx._version_snapshot = tuple(p._version for p in parents)
    out.grad_fn = fn

    return out


def add(a, b):
    a = enforce_tensor(a)
    b = enforce_tensor(b)
    return _wrap_forward(Add, a, b)


def sub(a, b):
    a = enforce_tensor(a)
    b = enforce_tensor(b)
    return _wrap_forward(Sub, a, b)


def mul(a, b):
    a = enforce_tensor(a)
    b = enforce_tensor(b)
    return _wrap_forward(Mul, a, b)


def div(a, b):
    a = enforce_tensor(a)
    b = enforce_tensor(b)
    return _wrap_forward(Div, a, b)


def neg(a):
    a = enforce_tensor(a)
    return _wrap_forward(Neg, a)


def matmul(a, b):
    a = enforce_tensor(a)
    b = enforce_tensor(b)
    return _wrap_forward(MatMul, a, b)


def tensor_sum(a, dim: int | tuple | None = None, keepdims: bool = False):
    a = enforce_tensor(a)
    return _wrap_forward(VariableSum, a, dim=dim, keepdims=keepdims)


def mean(a, dim=None, keepdims=False):
    a = enforce_tensor(a)
    return _wrap_forward(Mean, a, dim=dim, keepdims=keepdims)


def transpose(a):
    a = enforce_tensor(a)
    return _wrap_forward(Transpose, a)


def reshape(a, shape: tuple):
    a = enforce_tensor(a)
    return _wrap_forward(Reshape, a, shape=shape)


def pow(a, exponent):
    a = enforce_tensor(a)
    return _wrap_forward(Pow, a, exponent=exponent)


def exp(a):
    a = enforce_tensor(a)
    return _wrap_forward(Exp, a)


def log(a):
    a = enforce_tensor(a)
    return _wrap_forward(Log, a)


def index(a, idx):
    a = enforce_tensor(a)
    return _wrap_forward(Index, a, idx=idx)
